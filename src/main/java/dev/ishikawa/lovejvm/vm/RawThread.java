...
public class RawThread {
  ...
  private void stackUp(RawMethod nextMethod, int pcToReturn) {
    ...
    if (currentFrame() != null) {
      ...
      // TODO: Ideally, Not copying the words, but use the unified global stack per one
      // thread!!
    ...
    while (true) {
      ...
      switch (instruction) {
        ...
        case (byte) 0x11:
          { // sipush
            operandStack.push(
                Word.of(methodArea.lookupByte(pc + 1), methodArea.lookupByte(pc + 2)));
            pc = pc + 3;
            break;
          }
        case (byte) 0x3b:
        ...
        case (byte) 0xa2:
          { // if_icmpge
            ...
            int jumpTo =
                ByteUtil.concat(methodArea.lookupByte(pc + 1), methodArea.lookupByte(pc + 2));
            if (left >= right) {
              pc = pc + jumpTo;
            } else {
              pc = pc + 3;
